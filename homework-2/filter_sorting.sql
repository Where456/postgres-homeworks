...
SELECT order_date, shipped_date, ship_country, COUNT(*) FROM orders
...
SELECT AVG(EXTRACT(DAY FROM AGE(shipped_date, order_date)))FROM orders
...
SELECT MIN(unit_price) AS min_price, MAX(unit_price) FROM products
